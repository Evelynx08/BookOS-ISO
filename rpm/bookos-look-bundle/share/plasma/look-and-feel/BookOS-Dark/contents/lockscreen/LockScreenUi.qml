...
Item {
    ...
    MouseArea {
        ...
        // BookOS: isla dinámica (Live States) bajo el reloj
        LiveStatesIsland {
            dark: lockScreenRoot.conf_isDark
            anchors.horizontalCenter: parent.horizontalCenter
            anchors.top: parent.top
            anchors.topMargin: Math.round(lockScreenRoot.height * 0.08)
                               + Math.round(lockScreenRoot.height * 0.115)
                               + (lockScreenRoot.conf_showDate ? Math.round(lockScreenRoot.height * 0.032) : 0)
                               + 24
        }
